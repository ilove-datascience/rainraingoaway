...
async def begin_named_location(update, context, label):
    if update.effective_chat.type not in ('group', 'supergroup'):
        await update.message.reply_text('Multiple locations are available in groups only.')
        return ConversationHandler.END
    if not label or len(label) > 80:
        await update.message.reply_text('Use /addlocation followed by a name (up to 80 characters), for example /addlocation Office.')
        return ConversationHandler.END
    rows = await asyncio.to_thread(list_locations, update.effective_chat.id)
    if not rows:
        await update.message.reply_text('Use /start to set up the group’s Main location first.')
        return ConversationHandler.END
    if any(row['label'].casefold() == label.casefold() for row in rows):
        await update.message.reply_text('That location name is already saved. Choose another name.')
        return ConversationHandler.END
...
async def receive_extra_location(update, context):
    ...
    if not label:
        await update.message.reply_text('Use /addlocation followed by a name to begin again.')
        return ConversationHandler.END
    location = update.message.location
    if not in_coverage(location.latitude, location.longitude):
        await update.message.reply_text('That location is outside radar coverage. Send another location or /cancel.')
        return WAITING_FOR_EXTRA_LOCATION
...
async def locations_command(update, context):
    rows = await asyncio.to_thread(list_locations, update.effective_chat.id)
    if not rows:
        await update.message.reply_text('No saved locations. Use /start to set up this chat.')
        return
    # Separate short messages avoid Telegram’s message length limit.
    for row in rows:
        await update.message.reply_text(f"{row['label']}: {row['latitude']}, {row['longitude']}")
    if update.effective_chat.type in ('group', 'supergroup'):
        await update.message.reply_text('Use Add location or Remove location below. Change location updates Main.',
                                        reply_markup=main_menu(update.effective_chat.id))
...
async def remove_location_by_name(update, context, label):
    if not label:
        await update.message.reply_text('Use /removelocation followed by the saved name.')
        return
    async with settings_lock(context):
...
async def cancel_location(update, context):
    context.chat_data.pop('new_location_label', None)
    await update.message.reply_text('Setup cancelled.', reply_markup=main_menu(update.effective_chat.id))
    return ConversationHandler.END
...
async def add_location_button(update, context):
    ...
    rows = await asyncio.to_thread(list_locations, update.effective_chat.id)
    if not rows:
        await update.message.reply_text('Use /start to set up Main first.')
        return ConversationHandler.END
